...
from ritl.models.flag_store import FlagStore
    ...
    def connect(self) -> None:
        while True:
            ...
            try:
                ...
                log.info("Sensor TCP client connected")
                break
...
class _ActuationTcpServer:
    ...
    def _handle(self, conn: socket.socket) -> None:
        FRAME_SIZE = 9  # 1 byte CommandId + 8 bytes F64 deployment_level
        with conn:
            while True:
                try:
                    # Always read the full fixed 9-byte packet sent by F Prime - should be extended if additional fields are added
                    packet = b""
                    while len(packet) < FRAME_SIZE:
                        chunk = conn.recv(FRAME_SIZE - len(packet))
                        if not chunk:
                            raise ConnectionResetError("F Prime closed actuation connection")
                        packet += chunk

                    self._flag_store.actuate(ActuationCommand.from_bytes(packet))
...
class FPrimeAdapter(FswAdapter):
    ...
    def start(self) -> None:
        ...
        log.info("FPrimeAdapter started")

    def send_sensor(self, data: SensorData) -> bool:
        return self._client.send(data)

    def get_snapshot(self) -> ActuationData:
        return self._flag_store.snapshot()

    def wait_for_actuation(self, timeout: float = 0.2) -> ActuationData:
        return self._flag_store.wait_for_actuation(timeout)
