...
BARO_FREEZE_AT = 10.0  # seconds into flight to inject baro freeze
...
class RunnerConfig:
    ...
    name:                    str           = "ritl_fault_injection_experiment"
    ...
    def __init__(self):
        ...
        output.console_log("RITL Fault Injection RunnerConfig loaded")
    ...
    def create_run_table_model(self) -> RunTableModel:
        ...
        self.run_table_model = RunTableModel(
            ...
            repetitions=N_RUNS,
            data_columns=[
                "apogee_m", "apogee_time_s", "drogue_s",
                "main_s", "wall_time_s", "success"
            ],
        )
    ...
    def _log_path(self, mode: str) -> Path:
        return RITL_LOGS / f"sil_snapshot_{ROCKET}.log"
    ...
    def _patch_ritl_config(self, mode: str) -> None:
        ...
        if "fault_injection" not in cfg:
            cfg["fault_injection"] = {}
        cfg["fault_injection"]["enabled"]        = True
        cfg["fault_injection"]["freeze_baro"]    = True
        cfg["fault_injection"]["freeze_baro_at"] = BARO_FREEZE_AT
        cfg["fault_injection"]["dropout_rate"]   = 0.0

        with open(RITL_CONFIG, "w") as f:
            yaml.dump(cfg, f, default_flow_style=False)

        output.console_log(
            f"Config patched: arch=snapshot, fault=baro_freeze@{BARO_FREEZE_AT}s, "
            f"fsw_host={cfg['network']['fsw_host']}"
        )
    ...
    def _start_fprime(self, run_dir: Path) -> None:
        ...
        gds_log = open(run_dir / "gds.log", "w")
    ...
    def _kill_hil_fsw(self) -> None:
        ...
        subprocess.run(
            ["ssh", f"{HIL_SSH_USER}@{HIL_SSH_HOST}",
             f"pkill -f {HIL_FSW_BIN.split('/')[-1]} || true"],
            capture_output=True,
        )
    ...
    def before_experiment(self) -> None:
        output.console_log(f"Starting RITL fault injection experiment | runs={N_RUNS}")
        output.console_log(f"Baro freeze at t={BARO_FREEZE_AT}s")
    ...
    def populate_run_data(self, context: RunnerContext) -> Optional[Dict[str, SupportsStr]]:
        ...
        if parsed is None:
            return {
                "apogee_m": None, "apogee_time_s": None,
                "drogue_s": None, "main_s": None,
                "wall_time_s": None, "success": False,
            }

        dest_stem = f"{mode}_{run_id}"
        for f in RITL_LOGS.iterdir():
            if f.stem == log_path.stem:
    ...
    def after_experiment(self) -> None:
        output.console_log("Fault injection experiment complete.")
        output.console_log(
            f"Results saved to: {self.results_output_path / self.name}"
        )
