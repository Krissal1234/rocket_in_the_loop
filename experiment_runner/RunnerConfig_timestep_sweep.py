...
ROCKET = "cameos"
N_RUNS = 3             # nonsil is deterministic - reps are effectively free (no FSW startup)
RUN_TIMEOUT = 600       # seconds

TIME_STEPS = [0.0001, 0.001, 0.005, 0.01]   # seconds
FIXED_SAMPLE_RATE = 10.0             # Hz - held fixed, matching the main experiments

MODES = ["sil_snapshot", "nonsil"]
...
class RunnerConfig:
    ...
    def __init__(self):
        ...
        output.console_log(f"RITL Timestep Sweep RunnerConfig loaded (modes={MODES})")

    # ── Run table ─────────────────────────────────────────────────────────────
    def create_run_table_model(self) -> RunTableModel:
        mode_factor = FactorModel("mode", MODES)
        timestep_factor = FactorModel("time_step", TIME_STEPS)
        self.run_table_model = RunTableModel(
            factors=[mode_factor, timestep_factor],
            repetitions=N_RUNS,
    ...
    def _patch_ritl_config(self, mode: str, time_step: float) -> None:
        ...
        cfg["rocket_params"]["sample_rate"] = FIXED_SAMPLE_RATE
        cfg["rocket_params"]["time_step"]   = time_step
        ...
        output.console_log(
            ...
            f"sample_rate={FIXED_SAMPLE_RATE}Hz, time_step={time_step}s"
        )
    ...
    def before_experiment(self) -> None:
        output.console_log(f"Starting timestep sweep | modes={MODES} | time_steps={TIME_STEPS}s | "
                            f"sample_rate={FIXED_SAMPLE_RATE}Hz (fixed) | runs/cond={N_RUNS}")
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        time_step = context.execute_run["time_step"]
        output.console_log(f"Starting run: mode={mode}, time_step={time_step}s")
        self._patch_ritl_config(mode, time_step)
        if mode.startswith("sil"):
            self._kill_fprime()
    ...
    def interact(self, context: RunnerContext) -> None:
        ...
        try:
            ...
            output.console_log(f"ERROR: simulation timed out after {RUN_TIMEOUT}s "
                                f"- this may indicate solver instability at this time_step.")
    ...
    def populate_run_data(self, context: RunnerContext) -> Optional[Dict[str, SupportsStr]]:
        ...
        dest_stem = f"{mode}_ts{time_step}_{run_id}"
